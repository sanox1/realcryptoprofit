...
contract RCPFaucet is Ownable {
    ...
    uint256 public minHoldingBalance; // Minimum token balance required to mine

    event Mined(address indexed miner, uint256 amount, uint256 timestamp);
    event EmergencyWithdraw(address indexed owner, uint256 amount);
    event MinHoldingBalanceUpdated(uint256 oldBalance, uint256 newBalance); // NEW EVENT
    
	constructor(address _tokenAddress, uint256 _minHoldingBalance) Ownable() {
		token = IERC20(_tokenAddress);
		minHoldingBalance = _minHoldingBalance;
	}
	
	// Add setter function for owner to adjust minimum balance
	function setMinHoldingBalance(uint256 _newMinBalance) external onlyOwner {
		uint256 oldBalance = minHoldingBalance;
		minHoldingBalance = _newMinBalance;
		emit MinHoldingBalanceUpdated(oldBalance, _newMinBalance); // EMIT EVENT
	}
    
	function mine() external {
		// Check minimum token balance requirement
		uint256 minerBalance = token.balanceOf(msg.sender);
		require(minerBalance >= minHoldingBalance, "Must hold minimum token balance to mine");
    ...
    // View function to check if miner meets minimum balance requirement
    function meetsMinimumBalance(address miner) external view returns (bool) {
        return token.balanceOf(miner) >= minHoldingBalance;
    }
